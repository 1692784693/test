...
async def taoBaoLogin(username='18269411189', password='pang15878042803', url='https://login.taobao.com/member/login.jhtml?spm=a21bo.2017.754894437.1.5af911d9lphV9U&f=top&redirectURL=https%3A%2F%2Fwww.taobao.com%2F',keyword='美食'):
    #'headless': False如果想要浏览器隐藏更改False为True
    # 127.0.0.1:1080为代理ip和端口，这个根据自己的本地代理进行更改，如果是vps里或者全局模式可以删除掉'--proxy-server=127.0.0.1:1080'
    browser = await launch({'headless': False, 'args': ['--no-sandbox'],'timeout':0})
...
async def search(page,keyword):
    await page.waitForSelector('#J_TSearchForm')
    await page.type('#q',keyword,{'delay': input_time_random() - 50})
    #可以点击搜索
    await page.click('#J_TSearchForm > div.search-button > button')
    await page.waitFor(30000)
    page.mouse
    # await goods_pages(page)
    await getGoodsUrl(page)

#获取商品页
async def goods_pages(page,i=0):
    i = i + 1
    if i < 10:
        # await getGoodsUrl(page)
        await page.click('#mainsrp-pager > div > div > div > ul > li.item.next > a')
        page.waitFor(30000)
        page.mouse
        print('正在获取%s页的商品地址' % i )
        return await goods_pages(page,i)
    else:
        print('获取结束')
        return


async def getGoodsUrl(page):
    html = await page.content()
    soup = BeautifulSoup(html,'lxml')
    items = soup.select('#mainsrp-itemlist > div > div > div:nth-child(1) > div')
    for item in items:
        url = item.select('div.ctx-box.J_MouseEneterLeave.J_IconMoreNew > div.row.row-2.title > a')[0].get('href')
        if 'https:' not in url:
            url='https:'+url
            print(url)
            #await getGoodDetail(page,url)
        else:
            print(url)
            #await getGoodDetail(page,url)


async def getGoodDetail(page,url):
    pass
...
if __name__ == '__main__':
    ...
    loop.run_until_complete(taoBaoLogin())
